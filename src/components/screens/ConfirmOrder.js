...
export const ConfirmOrder = ({navigation,route})=>{
  const [result,setResult]=useState("");
  const {pickupLoc}=route.params;
  const {dropOffLoc}=route.params;
  const {receiver}=route.params;
  const {pickupLatitude}= route.params;
  const {pickupLongitude}=route.params;
  const {dropLatitude}= route.params;
  const {dropLongitude}=route.params;
  console.log("data recieved at cofirm order",route.params);
  /*const finduser=async()=>{
    const result=await AsyncStorage.getItem("user_email");
    console.log(result);
};
useEffect(()=>{
  finduser();
...
const {insertInDelivery, error,isLoading} = useContext(AuthenticationContext);
  return (
        ...
        <View style={styles.detailContainer}>
          <Text style={{ fontSize: 20, fontFamily: "QuickSand",lineHeight:30 }}>
            ...
            Pay at Pickup {"\n"}
             ...
             <TouchableOpacity style={styles.btn2} onPress={() => {insertInDelivery(result,x,date,pickupLoc,dropOffLoc,receiver,pickupLatitude,pickupLongitude,dropLatitude,dropLongitude); 
              navigation.navigate("Searching",{...route.params,uid:x})}}>
